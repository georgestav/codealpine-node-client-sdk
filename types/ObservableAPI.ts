...
export class ObservableUsersApi {
     ...
     * @param body 
     */
    public userControllerUpdate(id: string, body: any, options?: Configuration): Observable<string> {
    	const requestContextPromise = this.requestFactory.userControllerUpdate(id, body, options);
    ...
    let middlewarePreObservable = from<RequestContext>(requestContextPromise);
    	...
    	return middlewarePreObservable.pipe(mergeMap((ctx: RequestContext) => this.configuration.httpApi.send(ctx))).
	    	pipe(mergeMap((response: ResponseContext) => {
	    		...
	    		return middlewarePostObservable.pipe(map((rsp: ResponseContext) => this.responseProcessor.userControllerUpdate(rsp)));
	    	}));
    }
